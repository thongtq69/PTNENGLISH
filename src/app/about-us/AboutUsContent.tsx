...
function renderIcon(icon: any, className: string = "w-6 h-6") {
    if (!icon) return <Users className={className} />;
    if (React.isValidElement(icon)) {
        return React.cloneElement(icon as React.ReactElement<any>, { className: className });
    }
...
const POLICIES = [
    { title: "Thông tin công khai", icon: "ShieldCheck" },
    { title: "Điều khoản & Quy định", icon: "FileText" },
    { title: "Chính sách bảo mật", icon: "Lock" },
    { title: "Quy định học viên", icon: "UserCheck" }
];
...
export default function AboutUsContent({ pageData }: { pageData: any }) {
    ...
    const hero = sections.find((s: any) => s.type === 'about-hero')?.content || { title: "Kiến tạo hành trình", subtitle: "PTN English" };
    const story = sections.find((s: any) => s.type === 'about-story')?.content || { title: "Câu Chuyện Hình Thành", text: "..." };
    const teachers = sections.find((s: any) => s.type === 'about-teachers')?.content?.items || TEACHERS;
        ...
        <main className="min-h-screen bg-white">
            ...
            <section className="py-20 bg-slate-100 overflow-hidden relative">
                ...
                <div className="container mx-auto px-6 relative z-10">
                    ...
                    <DifferencesHub items={differencesData} />
                </div>
            </section>
                    ...
                    <div className="bg-slate-100 rounded-[2.5rem] p-8 md:p-12 border border-slate-200 flex flex-col lg:grid lg:grid-cols-12 gap-10 items-stretch">
                        ...
                        <div className="lg:col-span-12 xl:col-span-5 flex flex-col justify-center">
                            <div className="mb-8 lg:mb-0">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-1 gap-4">
                                    {philosophyData.map((item: any, idx: number) => (
                                        <motion.div
                                            key={idx}
                        ...
                        <div className="lg:col-span-12 xl:col-span-7 flex flex-col justify-center">
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-2 gap-3">
                                {valuesData.map((val: any, idx: number) => (
                                    <motion.div
                                        key={idx}
                                        whileHover={{ y: -3 }}
            ...
            {/* Teachers Section - Refactored for Hierarchy */}
            <section id="teachers" className="py-32 bg-slate-50">
                <div className="container mx-auto px-6">
                    <div className="text-center max-w-3xl mx-auto mb-20">
                        ...
                        <h3 className="text-4xl md:text-5xl font-heading font-extrabold text-accent leading-tight">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-32">
                        {teachers.slice(0, 3).map((teacher: any, idx: number) => (
                            <motion.div
                                key={idx}
                                initial={{ opacity: 0, y: 30 }}
                            ...
                            >
                                <div className="relative aspect-[3/4] rounded-[2.5rem] overflow-hidden shadow-2xl bg-white border border-slate-100">
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 p-8 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                                        ...
                                        <div className="space-y-3">
                                            <p className="text-white/70 text-xs leading-relaxed italic border-l-2 border-primary/50 pl-4">
                                                {teacher.desc}
                                            </p>
                                        </div>
                                    </div>

                                    {/* Role Badge for Founders */}
                                    <div className="absolute top-6 right-6 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white text-[9px] font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <div className="mt-8 text-center">
                                    ...
                                    <div className="flex items-center justify-center gap-4 mt-3">
                                        ...
                                        <p className="text-[11px] text-primary font-black uppercase tracking-widest">{teacher.exp}</p>
                                        <span className="h-px w-8 bg-slate-200" />
                                    </div>
                                </div>
                            </motion.div>
                    ...
                    {teachers.length > 3 && (
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                {teachers.slice(3).map((teacher: any, idx: number) => (
                                    <motion.div
                                        key={idx}
                                    ...
                                    >
                                        <div className="relative aspect-[3/4] rounded-[2rem] overflow-hidden shadow-lg bg-white border border-slate-100 grayscale hover:grayscale-0 transition-all duration-500">
                                            ...
                                            <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-accent to-transparent">
                                                <p className="text-white text-[10px] font-bold leading-tight line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                                    {teacher.certs}
                                                </p>
                                            </div>
                                        </div>
...
function DifferencesHub({ items }: { items: any[] }) {
        ...
        <div className="relative min-h-[550px] flex items-center justify-center scale-100 md:scale-110">
                    ...
                    {items.map((_, idx) => {
                        ...
                        return (
                            <motion.line
                                ...
                                strokeWidth={hovered === items[idx].id ? "2" : "1"}
                                initial={{ pathLength: 0, opacity: 0 }}
                                animate={{ pathLength: 1, opacity: hovered === items[idx].id ? 0.8 : 0.2 }}
            ...
            <div className="lg:hidden w-full flex flex-col gap-8">
                ...
                <div className="bg-white border-2 border-slate-100 p-8 rounded-[2rem] shadow-xl relative min-h-[300px]">
                    <AnimatePresence mode="wait">
                        {(() => {
                            const activeItem = items.find(d => d.id === (hovered || 1));
                            return (
                                <motion.div
                                    key={activeItem?.id}
                                ...
                                >
                                    <div className="flex items-center gap-4 mb-6">
                                        <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary">
                                            {renderIcon(activeItem?.icon, "w-5 h-5")}
                                        </div>
