...
const TABS = [
    ...
    { id: 'messages', name: 'Student Messages', icon: <Quote size={18} /> },
];
        ...
        fetch('/api/pages/student-corner')
            ...
            .then(d => {
                ...
                const normalized = {
                    ...
                    messages: d.sections.find((s: any) => s.type === 'student-messages')?.content || { title: '', subtitle: '', description: '', notes: [] }
                };
                setData(normalized);
                setLoading(false);
            })
            .catch(() => {
                ...
                setData({
                    ...
                    messages: {
                        ...
                        notes: [
                            { id: 1, image: "https://scontent.fsgn2-6.fna.fbcdn.net/v/t39.30808-6/526743323_122171120402241571_3498967980315481358_n.jpg", pinColor: "#ef4444", rotation: -3 },
                            { id: 2, image: "https://scontent.fsgn2-11.fna.fbcdn.net/v/t39.30808-6/526743323_122171120402241571_3498967980315481358_n.jpg", pinColor: "#22c55e", rotation: 2 },
                        ]
                    }
                });
                setLoading(false);
    ...
    const handleSave = async () => {
        ...
        try {
            const sections = [
                ...
                { id: 'student-messages', type: 'student-messages', content: data.messages, order: 6, isVisible: true },
            ];
                    ...
                    {activeTab === 'messages' && (
                        <div className="space-y-8">
                            <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-10 space-y-8 shadow-2xl">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-4">
                                        <RichTitleEditor
                                            label="Section Title"
                                            value={data.messages.title}
                                            onChange={val => setData({ ...data, messages: { ...data.messages, title: val } })}
                                        />
                                    </div>
                                    <div className="space-y-4">
                                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest">Subtitle Tag</label>
                                        <input
                                            value={data.messages.subtitle}
                                            onChange={e => setData({ ...data, messages: { ...data.messages, subtitle: e.target.value } })}
                                            className="w-full bg-slate-950 border border-white/10 rounded-2xl px-6 py-4 text-slate-300 outline-none"
                                        />
                                    </div>
                                    <div className="md:col-span-2 space-y-4">
                                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest">Section Description</label>
                                        <textarea
                                            rows={2}
                                            value={data.messages.description}
                                            onChange={e => setData({ ...data, messages: { ...data.messages, description: e.target.value } })}
                                            className="w-full bg-slate-950 border border-white/10 rounded-2xl px-6 py-4 text-slate-400 font-body outline-none"
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-6">
                                {data.messages.notes.map((note: any, idx: number) => (
                                    <div key={note.id || idx} className="bg-slate-900 border border-white/5 p-6 rounded-[2rem] space-y-4 group relative overflow-hidden">
                                        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                            <button onClick={() => {
                                                const list = data.messages.notes.filter((_: any, i: number) => i !== idx);
                                        ...
                                        <div className="space-y-3">
                                            <FileUpload
                                                label="Upload Message Image"
                                                value={note.image}
                                                onChange={(url) => {
                                                    const list = [...data.messages.notes];
                                                    list[idx].image = url;
                                                    setData({ ...data, messages: { ...data.messages, notes: list } });
                                                }}
                                            ...
                                            <div className="grid grid-cols-2 gap-3">
                                                <div>
                                                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest block mb-1">Pin Color</label>
                                                    <input
                                                        type="color"
                                                        value={note.pinColor}
                                                        onChange={e => {
                                                            const list = [...data.messages.notes];
                                                            list[idx].pinColor = e.target.value;
                                                            setData({ ...data, messages: { ...data.messages, notes: list } });
                                                        }}
                                                        className="w-full h-10 rounded-lg bg-slate-950 border border-white/10 cursor-pointer"
                                                    />
                                                </div>
                                                <div>
                                                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest block mb-1">Rotation</label>
                                                    <input
                                                        type="number"
                                                        value={note.rotation}
                                                        onChange={e => {
                                                            const list = [...data.messages.notes];
                                                            list[idx].rotation = parseInt(e.target.value);
                                                            setData({ ...data, messages: { ...data.messages, notes: list } });
                                                        }}
                                                        className="w-full h-10 rounded-lg bg-slate-950 border border-white/10 px-3 text-white text-xs"
                                                    />
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <button
                                    onClick={() => setData({ ...data, messages: { ...data.messages, notes: [...data.messages.notes, { id: Date.now(), image: '', pinColor: '#ef4444', rotation: 0 }] } })}
                                    className="border-2 border-dashed border-white/10 rounded-[2rem] flex flex-col items-center justify-center p-10 hover:border-primary/50 hover:bg-white/[0.02] transition-all text-slate-600 aspect-square"
                                >
                                    <Plus size={32} className="mb-2" />
                                    <span className="font-black text-[10px] uppercase tracking-widest text-center">Add Student Message</span>
                                </button>
                            </div>
                        </div>
                    )}
