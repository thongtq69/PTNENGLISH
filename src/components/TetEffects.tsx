...
// ── Hoa Mai rơi (only universally supported emoji) ──
const FALLING_ITEMS = ["🌸", "🌼", "🧧", "⭐", "🎊", "🌺"];
...
// ── 🏇 Ngựa phi ngang màn hình (always left→right) ──
const GallopingHorse = memo(function GallopingHorse() {
  const [horses, setHorses] = useState<{ id: number; y: number; speed: number; size: number }[]>([]);
