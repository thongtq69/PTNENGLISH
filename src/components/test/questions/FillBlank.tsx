...
export const FillBlank = memo(function FillBlank({ qIdx, value, onChange, inputRef }: FillBlankProps) {
  return (
    ...
    >
      <div
        className={`w-7 h-full flex items-center justify-center text-[10px] font-black border-r ${value ? "bg-emerald-500 text-white border-emerald-500" : "bg-slate-50 text-slate-400 border-slate-200"
          }`}
      >
        {qIdx}
      </div>
      <input
        type="text"
        autoComplete="off"
        className="flex-1 bg-transparent outline-none px-2 py-0 text-[13px] font-bold text-slate-800 w-24 md:w-32 placeholder:text-slate-300 placeholder:font-normal h-full"
        value={value}
